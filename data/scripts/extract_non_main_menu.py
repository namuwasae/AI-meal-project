...
# 메인메뉴를 제외한 데이터 추출
non_main_menu_df = df[~df['식품명'].isin(main_menus)]
...
# 데이터 저장
output_filename = f"{output_dir}/non_main_menu.xlsx"
non_main_menu_df.to_excel(output_filename, index=False)

# 데이터 요약 출력
print(f"메인메뉴 제외 데이터 파일이 생성되었습니다: {output_filename}")
print(f"총 {len(non_main_menu_df)}개의 메뉴가 추출되었습니다.")

# 조리방법별 메뉴 수 출력
print("\n조리방법별 메뉴 수:")
method_counts = non_main_menu_df['식품대분류명'].value_counts()
